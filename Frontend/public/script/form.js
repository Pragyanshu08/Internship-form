...
while (
    ...
    while (savedData[`company_name${ExperienceIndex}`]) {
      ...
      section.innerHTML = `
        ...
        <input type="text" name="Position${ExperienceIndex}">
         ...
         <label for="experience_desc${ExperienceIndex}">Description</label>
    <textarea name="experience_desc${ExperienceIndex}" id="experience_desc${ExperienceIndex}" rows="1" cols="40">${savedData[`desc${ExperienceIndex}`] || ''}</textarea>
  ...
  experienceBtn.addEventListener("click", () => {
    ...
    section.innerHTML = `
        ...
        <label for="experience_desc${window.experienceCount}">Description</label>
        <textarea name="experience_desc${window.experienceCount}" id="experience_desc${window.experienceCount}" rows="1" cols="40"></textarea> 
...
document.getElementById("resume").addEventListener("change", function () {
  ...
  experienceSections.forEach(section => {
    ...
    const desc = section.querySelector('textarea[name^="experience_desc"]')?.value || "";
    const duration = section.querySelector('input[name^="duration"]')?.value || "";


   if (company_name && position && duration) {
      experiences.push({ company_name, position, experience_desc: desc ,duration });
    }
  });
